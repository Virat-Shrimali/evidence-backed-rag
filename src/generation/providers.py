...
class MockLLMProvider(BaseLLMProvider):
    ...
    def generate_raw(self, prompt: str, system_prompt: str) -> str:
        ...
        # Detect refusal / unanswerable trigger in prompt
        if any(w in prompt.lower() for w in ["unanswerable", "redis", "elasticsearch", "postgresql connection"]):
            return json.dumps({
        ...
        # Extract first chunk ID and a real snippet from context in prompt
        chunk_id_match = re.search(r"--- Chunk ID: ([\w\-#]+) ---", prompt)
        chunk_id = chunk_id_match.group(1) if chunk_id_match else "mock_chunk_0"

        # Find first non-empty line of content
        content_lines = [
            line.strip()
            for line in prompt.split("Context:\n")[-1].split("\n\nQuestion:")[0].splitlines()
        ...
        return json.dumps({
            "answer": f"Based on the provided documentation, {snippet}.",
            "citations": [{"chunk_id": chunk_id, "text_snippet": snippet}],
